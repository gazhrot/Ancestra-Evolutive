...
public class Player extends Creature {
	...
	public boolean boostSpell(int id) {
		if(this.getSortStatBySortIfHas(id)== null) {
			Log.addToLog(this.getName()+" ne possï¿½de pas le sort d'id : " + id + " !");
			return false;
		}
	...
	public boolean forgetSpell(int id) {
		if(getSortStatBySortIfHas(id)== null) {
			if(Server.config.isDebug()) Log.addToLog(this.getName()+" ne possï¿½de pas le sort d'id " + id + " !");
			return false;
		}
	...
	public void onJoinGame() {
		...
		SocketManager.GAME_SEND_ADD_CANAL(out, this.canaux + "^" + (this.getAccount().getGmLvl() > 0 ? "@ï¿½" : ""));
	...
	public int learnJob(Job m)
	{
		...
		this.jobs.put(pos, sm);//On apprend le mï¿½tier lvl 1 avec 0 xp
		if(this.isOnline)
		{
		...
		switch(c) {
			case '*':
				if(this.getDeshonor() > 0) {
					SocketManager.GAME_SEND_MESSAGE(this, "Vous avez " + this.getDeshonor() + " point"+(this.getDeshonor() == 1 ? "" : "s")+ "de dï¿½shonneur. Action impossible.", Server.config.getMotdColor());
					return;
				}

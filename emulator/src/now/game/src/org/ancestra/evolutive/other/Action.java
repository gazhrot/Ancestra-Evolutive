	...
	public void apply(Player perso, Player target, int itemID, int cellid)
	{
		...
		switch(this.id)
		{
			...
			case -1://Ouvrir banque
				...
				perso.getGameActionManager().setStatus(GameActionManager.Status.EXCHANGING);
			...
			case 11://Definir l'alignement
				try
				{
					...
					if(perso.getAlignement() != Alignement.NEUTRE && !replace)return;
			...
			case 50://Traque
				...
				if(perso.getStalk().getTime() < System.currentTimeMillis() - 600000 || perso.getStalk().getTime() == 0)
				{
					...
					if(P.getAlignement() == perso.getAlignement() || P.getAlignement() == Alignement.NEUTRE || P.getAlignement() == Alignement.MERCENAIRE)continue;

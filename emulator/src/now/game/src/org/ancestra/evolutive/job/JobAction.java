...
public class JobAction {
	...
	public void startAction(Player P, InteractiveObject IO, int GA, Case cell, JobStat SM)
	{
		this.SM = SM;
		this.player = P;
		...
		if(!this.isCraft)
		{
			...
			SocketManager.GAME_SEND_GA_PACKET_TO_MAP(P.getMap(),""+GA, 501, P.getId()+"", cell.getId()+","+this.time);
		...
		{
			P.getGameActionManager().setStatus(GameActionManager.Status.CRAFTING);
	...
	public void endAction(Player P, InteractiveObject IO, int GA, Case cell)
	{
		P.setInAction(false);
		if(!this.isCraft)
		{
		...
		P.getGameActionManager().resetActions();
	}
	...
	public void craft()	
	{
		...
		for(Entry<Integer,Integer> e : this.ingredients.entrySet())
		{
			...
			Object obj = World.data.getObject(e.getKey());
			if(obj == null)
			{
				SocketManager.GAME_SEND_Ec_PACKET(this.player,"EI");
			...
			if(newQua == 0)
			{
				...
				World.data.removeObject(e.getKey());
		...
		{
			Object newObj = World.data.getObjectTemplate(tID).createNewItem(1, false);
	...
	public void putLastCraftIngredients() {
		...
		for (Entry<Integer, Integer> e : this.ingredients.entrySet())
		{
			if (World.data.getObject(e.getKey()) == null)
				return;
			if (World.data.getObject(e.getKey()).getQuantity() < e.getValue())
				return;
	...
	private void doFmCraft() {
		...
		for (int idIngredient : this.ingredients.keySet()) {
			Object ing = World.data.getObject(idIngredient);
				...
				default :
					...
					|| ing.getTemplate().getPaCost() > 0) {
						...
						if (objectFm.getQuantity() > 1) {
							...
							World.data.removeObject(idIngredient);
		...
		if (!sucess) { // Si il n'a pas r�ussi
			if (signingRune != null) {
				...
				if (newQua <= 0) {
					...
					World.data.removeObject(signingRune.getId());
			...
			if (runeOrPotion != null) {
				...
				if (newQua <= 0) {
					...
					World.data.removeObject(runeOrPotion.getId());
				...
				World.data.addObject(objectFm, true);
				this.player.addObject(objectFm);
				if (!statStringObj.isEmpty()) {
	...
	public static int WeithTotalBase(int objTemplateID)
	{
		...
		statsTemplate = World.data.getObjectTemplate(objTemplateID).getStrStats();

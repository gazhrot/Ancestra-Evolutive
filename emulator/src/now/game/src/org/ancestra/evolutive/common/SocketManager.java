...
public class SocketManager {
	...
	/**
	 * APE2 : Cache le bouton de g�n�ration de pseudo � la cr�ation de personnage.
	 * 
	 * 
	 */
	...
	public static void GAME_SEND_GTM_PACKET_TO_FIGHT(Fight fight, int teams)
	{
		...
		for(Fighter f : fight.getFighters(3))
		{
			...
			packet.append("0;").append(f.getPDV()+";").append(f.getDefaultPA()+";").append(f.getDefautPM()+";");
	...
	{
		long xpMin = World.data.getExpLevel(g.getLevel()).guild;
		long xpMax;
		...
		{
			xpMax = World.data.getExpLevel(g.getLevel()+1).guild;
		}
		StringBuilder packet = new StringBuilder();

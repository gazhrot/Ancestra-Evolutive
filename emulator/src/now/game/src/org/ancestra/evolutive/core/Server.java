...
public class Server {
	...
	//login database
	private String lhost, luser, lpass;
	private int lport;
	private String ldatabaseName;
	...
	private boolean nameGenerator, socketUseCompactData = false;
    ...
    public void initialize() {
		try {
			...
			//database
			this.lhost = configFile.getString("database.login.host");
			this.luser = configFile.getString("database.login.user");
			this.lpass = configFile.getString("database.login.password");
			this.ldatabaseName = configFile.getString("database.login.databaseName");
            this.lport = configFile.getInt("database.login.port");
            
			this.host = configFile.getString("database.game.host");
			this.user = configFile.getString("database.game.user");
			this.pass = configFile.getString("database.game.password");
			this.databaseName = configFile.getString("database.game.databaseName");
            this.port = configFile.getInt("database.game.port");
			...
			this.nameGenerator = configFile.getBoolean("network.nameGenerator");
			this.serverId = configFile.getInt("network.server.id");
			this.serverKey = configFile.getString("network.server.key");
			...
			this.useSubscribe = configFile.getBoolean("server.useSubscribe");
	...
	public void initializeCommands() {
		...
		if(configFile.getBoolean("commands.players.teleport.savePos.active")) {
			...
			Command<Player> command = new Command<Player>(name, "T�l�porte votre joueur � votre derni�re position sauvegarder.", null) {
		...
		if(configFile.getBoolean("commands.players.list.commandList.active")) {
			String name = configFile.getString("commands.players.list.commandList.name").toUpperCase();
			
			//creation de la commande
			Command<Player> command = new Command<Player>(name, "Affiche toute les commandes disponible.", null) {
    ...
    public String getLDatabaseName() {
    ...
    public int getIaThreadQuantity() {
        return 1;
    }
}

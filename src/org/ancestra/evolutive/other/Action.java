		...
		switch(this.id)
		{
			...
			case 12://Spawn d'un groupe de monstre
				try
				{
					...
					if(inArena && !World.data.isArenaMap(perso.getCurMap().getId()))return;	//Si la map du personnage n'est pas classé comme étant dans l'arène

					SoulStone pierrePleine = (SoulStone)World.data.getObjet(itemID);

					String groupData = pierrePleine.parseGroupData();
					String condition = "MiS = "+perso.getUUID();	//Condition pour que le groupe ne soit lançable que par le personnage qui à utiliser l'objet
			...
			case 18://Téléportation chez sois
				...
				{
					Object obj = World.data.getObjet(itemID);
			...
			case 25://SimpleUnMorph
				int UnMorphID = perso.getClasse()*10 + perso.getSex();
			...
			case 50://Traque
				...
				if(perso.getStalk().getTime() < System.currentTimeMillis() - 600000 || perso.getStalk().getTime() == 0)
				{
					...
					ObjectTemplate T = World.data.getObjTemplate(10085);
					if(T == null)return;

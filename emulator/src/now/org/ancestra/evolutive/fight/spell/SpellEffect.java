...
public class SpellEffect {
	...
	private Case cell = null;
					...
					switch(id)
					{
						case 9://Derobade
							...
							if (newCellIDD < 0) {
								...
								int nextCellID = 0;
								if (newCellIDD == 0)
									nextCellID = Pathfinding.GetCaseIDFromDirrection(target.getCell().getId(), dir, fightmap, true);
								else
							...
							if (affected != null) {
								...
								if (finalDamage > 0) {
									...
									SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, caster.getId() + "", affected.getId() + ",-"
											+ finalDamage);
									if (affected.getPDV() <= 0) {
										fight.onFighterDie(affected);
									}
								}
							}
							try {
		...
		private void applyEffect_783(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			int tcellID = Pathfinding.GetCaseIDFromDirrection(ccase.getId(), d, fight.getMap(), true);
			...
			}else if(turns <= 0)
			{
				...
				for(Fighter target : cibles)
				{
					...
					}else if(chance > 0 && spell==108)//Esprit f�lin ?
					{
						...
						fDommage = -(fDommage);
						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, caster.getId() + "", caster.getId() + "," + fDommage);
						if(caster.getPDV() <=0)
							fight.onFighterDie(caster);
					}else
					{

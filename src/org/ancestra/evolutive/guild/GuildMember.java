...
public class GuildMember {
	...
	public void setAllRights(int rank, byte xp, int right) {
		...
		if(right != this.getRight() && right != 1)//Vérifie si les droits sont pareille ou si des droits de meneur; pour ne pas faire la conversion pour rien
	...
	public void parseIntToRight(int total) {
		...
		if(this.getRights().size() > 0)//Si les droits contiennent quelque chose -> Vidage (Même si le TreeMap supprimerais les entrées doublon lors de l'ajout)
			this.getRights().clear();
			
		initRight();
		
		Integer[] mapKey = this.getRights().keySet().toArray(new Integer[this.getRights().size()]);//Récupère les clef de map dans un tableau d'Integer

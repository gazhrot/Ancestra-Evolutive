...
import fr.edofus.ancestra.evolutive.objects.SpellEffect;
...
public class JobAction {
	...
	public void startAction(Player P, InteractiveObject IO, GameAction GA, Case cell, JobStat SM)
	{
		...
		if(P.getObjetByPos(Constants.ITEM_POS_ARME) != null && SM.getTemplate().getId() == 36)
		{
			...
			{
				int dist = Pathfinding.getDistanceBetween(P.get_curCarte(), P.get_curCell().getID(), cell.getID());
				int distItem = 0;
                  	...
                  	if(tID == protectors[i][1])
                   	{
                     	...
                     	int monsterLvl = JobConstant.getProtectorLvl(P.get_lvl());            
                      	P.get_curCarte().startFightVersusProtectors(P, new MobGroup(P.get_curCarte()._nextObjectID, cell.getID(), monsterId+","+monsterLvl+","+monsterLvl));
                        break;
                 	}
                }
			}
	...
	public void craft()	
	{
		...
		for(Entry<Integer,Integer> e : this.ingredients.entrySet())
		{
			...
			if(!this.player.hasItemGuid(e.getKey()))
			{
				...
				Log.addToSockLog("/!\\ "+this.player.get_name()+" essaye de crafter avec un objet qu'il n'a pas");
				return;
			}
			...
			{
				SocketManager.GAME_SEND_Ec_PACKET(this.player,"EI");
				Log.addToSockLog("/!\\ "+this.player.get_name()+" essaye de crafter avec un objet qui n'existe pas");
				return;
			}
			...
			{
				SocketManager.GAME_SEND_Ec_PACKET(this.player,"EI");
				Log.addToSockLog("/!\\ "+this.player.get_name()+" essaye de crafter avec un objet dont la quantite est trop faible");
				return;
			}
		...
		{
			SocketManager.GAME_SEND_Ec_PACKET(this.player,"EI");
			SocketManager.GAME_SEND_IO_PACKET_TO_MAP(this.player.get_curCarte(),this.player.get_GUID(),"-");
		...
		if(!success)//Si echec
		{
			...
			SocketManager.GAME_SEND_IO_PACKET_TO_MAP(this.player.get_curCarte(),this.player.get_GUID(),"-"+tID);
		...
		}else
		{
			...
			if(signed)newObj.addTxtStat(988, this.player.get_name());
			boolean add = true;
	...
	private void doFmCraft() {
		...
		if (!sucess) { // Si il n'a pas réussi
			...
			SocketManager.GAME_SEND_IO_PACKET_TO_MAP(this.player.get_curCarte(), this.player.get_GUID(), "-" + objTemaplateID);
			SocketManager.GAME_SEND_Ec_PACKET(this.player, "EF");
			...
			if (lvlElementRune == 1)
				...
				coef = 85;
			if (isSigningRune) {
				objectFm.addTxtStat(985, this.player.get_name());
			}
			if (lvlElementRune > 0 && lvlQuaStatsRune == 0) {

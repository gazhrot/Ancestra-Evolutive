...
public class JobStat {
	...
	public boolean startAction(int id, Player P, InteractiveObject IO, int GA, Case cell) {
	...
	public void endAction(int actionId, Player P, InteractiveObject IO, Case cell)
	{
		if(this.curAction == null) {
			SocketManager.GAME_SEND_MESSAGE(P, "Erreur action en cours null.", Server.config.getMotdColor());
			return;
		}
		this.curAction.endAction(P,IO,actionId,cell);
	...
	public void addXp(Player P,long xp)
	{
		...
		while(this.xp >= World.data.getExpLevel(this.lvl+1).job && this.lvl <100)
			levelUp(P,false);	
		//s'il y a eu Up
	...
	public String getXpString(String s)
	{
		...
		str.append(World.data.getExpLevel(this.lvl).job).append(s);
		str.append(this.xp).append(s);
		str.append(World.data.getExpLevel((this.lvl<100?this.lvl+1:this.lvl)).job);
		return str.toString();
	}

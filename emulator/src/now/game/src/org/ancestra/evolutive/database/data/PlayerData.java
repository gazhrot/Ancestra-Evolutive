...
public class PlayerData extends AbstractDAO<Player>{
	...
	public PlayerData(HikariDataSource source) {
		super(source);
	...
	public boolean create(Player obj) {
		String baseQuery = "INSERT INTO players( `id` ," +
	...
	public boolean delete(Player obj) {
		try {
			String baseQuery = "DELETE FROM players WHERE id = "+obj.getId();
			execute(baseQuery);
	...
	public boolean update(Player obj)  {
		try {
			String baseQuery = "UPDATE `players` SET `kamas`= ?,`spellboost`= ?,"+
	...
	public Player load(int id) {
		Player player = null;
		try {
			Result result = getData("SELECT * FROM players WHERE id = "+id);
    ...
    public void load() {
        try {
            Result result = getData("SELECT * FROM players");
	...
	public ArrayList<Player> load(Account obj) {
		...
		try {
			Result result = getData("SELECT * FROM players WHERE account = "+obj.getUUID());
            Player player;
	...
	public boolean exist(String name) {
		boolean exist = false;
		try {
			Result result = getData("SELECT * FROM players WHERE name = '"+name+"'");
	...
	public boolean updateSex(Player obj) {
		try {
			String baseQuery = "UPDATE `players` SET `sexe`= ? WHERE `players`.`id` = ? LIMIT 1;";
	...
	public String haveOtherPlayer(int account) {
		...
		try {
			String query = "SELECT server FROM players WHERE account = '"+account+"' AND NOT server = '" + Server.config.getServerId() + "';";
			Result result = getData(query);
			while(result.resultSet.next())
				servers += (servers.isEmpty() ? result.resultSet.getInt("server") : "," + result.resultSet.getInt("server"));
			close(result);
            logger.trace("load other player on account {}", account);
		} catch (Exception e) {
            logger.info("Can t load other player on account {}", account);
		}
		return servers;
	}
}

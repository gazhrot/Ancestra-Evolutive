...
public class SpellEffect {
	...
	private Case cell = null;
		...
		public int getMaxMinSort(Fighter target, int valeur) {
			int val = valeur;
			...
			if (target.hasBuff(781))
				valeur = Integer.parseInt(args.split(";")[0]);
			return val;
		}
				...
				for(SpellEffect buff : target.getBuffsByEffectID(id))
				{
					switch(id)
					{
						case 9://Derobade
							...
							int dist = Pathfinding.getDistanceBetween(fight.getMap(), target.getCell().getId(), caster.getCell().getId());
							if (dist > 1)
								continue;
							int cellNumbers = 0;
							try {
							...
							if (cellNumbers == 0 || target.isState(6))
								continue;
							Case aCell = caster.getCell();
							Fighter affected = null;
							Maps fightmap = fight.getMap();
							int newCellIDD = Pathfinding.getCellAfterPushDerobade(fightmap, aCell, target.getCell(), cellNumbers,
									fight, target);
							if (newCellIDD == 0)
								continue;
							finalDommage = 0;
							if (newCellIDD < 0) {
								...
								float b = (caster.getLvl() / (float) (100.00));
								if (b < 0.1)
									b = 0.1f;
								float C = b * a;
								...
								a = cellNumbers - a;
								newCellIDD = Pathfinding.getCellAfterPushDerobade(fightmap, aCell, target.getCell(), a, fight, target);
								char dir = Pathfinding.getDirBetweenTwoCase(aCell.getId(), target.getCell().getId(), fightmap, true);
								int nextCellID = 0;
								if (newCellIDD == 0)
									nextCellID = Pathfinding.GetCaseIDFromDirection(target.getCell().getId(), dir, fightmap, true);
								else
									nextCellID = Pathfinding.GetCaseIDFromDirection(newCellIDD, dir, fightmap, true);
								Case nextCell = fightmap.getCases().get(nextCellID);
								if (nextCell != null) {
									affected = nextCell.getFirstFighter();
								}
							}
							if (newCellIDD != 0) {
								Case newCell = fightmap.getCases().get(newCellIDD);
								if (newCell != null) {
									target.getCell().getFighters().clear();
									target.setFightCell(newCell);
									newCell.addFighter(target);
									SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 5, caster.getId() + "", target.getId() + ","
											+ newCellIDD);
									try {
										Thread.sleep(500);
									} catch (Exception e) {}
							...
							if (finalDommage > 0)
								SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, caster.getId() + "", target.getId() + ",-"
										+ finalDommage);
							if (affected != null) {
							...
							for(Piege p : fight.getTraps()) {
								int dd = Pathfinding.getDistanceBetween(fight.getMap(), p.get_cell().getId(), newCellIDD);
								if (dd <= p.get_size()) {
									p.onTraped(target);
									break;
								}
						...
						case 788://Chatiments
							...
							if(stat == 108)
							{
								...
								SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, stat, caster.getId()+"", target.getId()+","+max+","+5);
								target.addPDVMAX(max);
		...
		private void applyEffect_51(Fight fight)
		{
			...
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950, target.getId()+"", target.getId()+","+Constants.ETAT_PORTE+",0");
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950, caster.getId()+"", caster.getId()+","+Constants.ETAT_PORTEUR+",0");
		}
		...
		private void applyEffect_50(Fight fight)
		{
			...
			target.setFightCell(caster.get_fightCell(false));
			...
			//on envoie les packets
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950, target.getId() + "", target.getId() + "," + Constants.ETAT_PORTE + ",1");
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 950, caster.getId()+"", caster.getId()+","+Constants.ETAT_PORTEUR+",1");
		...
		{
			//caster.addBuff(effectID, id, turns, 1, false, spell, args, caster);
			for(Fighter target : cibles)
			{ 
				target.addBuff(effectID, value, turns, 1, false, spell, args, target); 
			}
		...
		private void applyEffect_185(Fight fight)
		{
            ...
            int id = fight.getMap().getNextFreeId(IdType.CREATURE);
            Mob mob = new Mob(id,caster.getMap().getCases().get(cellID),MG);
            Fighter F = new Fighter(fight,mob,caster.getTeam());
			F.setInvocator(caster);
			fight.getMap().getCases().get(cellID).addFighter(F);
			F.setFightCell(fight.getMap().getCases().get(cellID));
		...
		private void applyEffect_672(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			int pdvMax = caster.getFightable().getMaxPdv();
		...
		private void applyEffect_783(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			char d = Pathfinding.getDirBetweenTwoCase(ccase.getId(),cell.getId(), fight.getMap(), true);
			//On calcule l'id de la case a cot� du lanceur dans la direction obtenue
			int tcellID = Pathfinding.GetCaseIDFromDirection(ccase.getId(), d, fight.getMap(), true);
			//on prend la case corespondante
			Case tcase = fight.getMap().getCases().get(tcellID);
			...
			while(true)
			{
				if(Pathfinding.GetCaseIDFromDirection(c1, d, fight.getMap(), true) == cell.getId())
	 				break;
				if(Pathfinding.GetCaseIDFromDirection(c1, d, fight.getMap(), true) == -1)
					return;
				c1 = Pathfinding.GetCaseIDFromDirection(c1, d, fight.getMap(), true);
				limite++;
				if(limite > 50)return;
			}
			...
			for(Piege p : P)
			{
				int dist = Pathfinding.getDistanceBetween(fight.getMap(),p.get_cell().getId(),target.get_fightCell(false).getId());
				//on active le piege
		...
		private void applyEffect_8(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			{
				int dist = Pathfinding.getDistanceBetween(fight.getMap(),p.get_cell().getId(),target.get_fightCell(false).getId());
				int dist2 = Pathfinding.getDistanceBetween(fight.getMap(),p.get_cell().getId(),caster.get_fightCell(false).getId());
				//on active le piege
				if(dist <= p.get_size())p.onTraped(target);
		...
		private void applyEffect_266(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_REM_CHAN, caster.getId()+"", target.getId()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Constants.STATS_ADD_CHAN, vol, turns, 1, false, spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_ADD_CHAN, caster.getId()+"", caster.getId()+","+vol+","+turns);
		}
		private void applyEffect_153(Fight fight, ArrayList<Fighter> cibles) {
			int val = Formulas.getRandomJet(jet);
			if (val == -1) {
			...
			int val2 = val;
			for (Fighter target : cibles) {
				val = getMaxMinSort(target, val);
				target.addBuff(effectID, val, turns, 1, true, spell, args, caster);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, effectID,
						caster.getId() + "", target.getId() + "," + val + ","
								+ turns);
				val = val2;
			}
			try {
		...
		private void applyEffect_156(Fight fight, ArrayList<Fighter> cibles) {
			int val = Formulas.getRandomJet(jet);
			if (val == -1) {
				Log.addToLog("Erreur de valeur pour getRandomJet (applyEffect_"+effectID+")");
				return;
		...
		private void applyEffect_164(ArrayList<Fighter> cibles, Fight fight) {
			int val = value;
			if (val == -1) {
				Log.addToLog("Erreur de valeur pour getRandomJet (applyEffect_"+effectID+")");
				return;
			...
			for (Fighter target : cibles) {
				target.addBuff(effectID, val, turns, 1, false, spell, args,
						caster);
			}
			try {
				Thread.sleep(200);
		...
		private void applyEffect_220(ArrayList<Fighter> cibles, Fight fight) { 
			if (turns < 1)
				return;
			else {
				for (Fighter target : cibles)
					target.addBuff(effectID, 0, turns, 0, true, spell, args,
							caster);
			}
			try {
				Thread.sleep(200);
		...
		private void applyEffect_786(ArrayList<Fighter> cibles, Fight fight) {
			for (Fighter target : cibles)
				target.addBuff(effectID, value, turns, 1, true, spell, args,
						caster);
			try {
				Thread.sleep(200);
			} catch (InterruptedException e1) {
		...
		private void applyEffect_671(ArrayList<Fighter> cibles, Fight fight) {
				if (turns <= 0) {
					...
					int dom = Formulas.getRandomJet(args.split(";")[5]);
					dom= getMaxMinSort(target, dom);
					int val = caster.getPDV() / 100 * dom;
					val -= resF;
					int reduc = (int) (((float) val) / (float) 100) * resP;
					val -= reduc;
					if (val < 0)
						val = 0;
					val = applyOnHitBuffs(val, target, caster, fight);
					if (val > target.getPDV())
						val = target.getPDV();
					target.removePDV(val);
					int soin = val;
					if (target.hasBuff(786) && target.getBuff(786) != null) {
						if ((soin + caster.getPDV()) > caster.getPDVMAX())
							soin = caster.getPDVMAX() - caster.getPDV();
						caster.removePDV(-soin);
						SocketManager
								.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100,
										target.getId() + "", caster.getId()
												+ ",+" + soin);
					}
					val = -(val);
		...
		private void applyEffect_776(ArrayList<Fighter> cibles, Fight fight) {
			int val = Formulas.getRandomJet(jet);
			if (val == -1) {
				Log.addToLog("Erreur de valeur pour getRandomJet (applyEffect_"+effectID+")");
				return;
		...
		private void applyEffect_143(ArrayList<Fighter> cibles, Fight fight) {
			if (turns <= 0) {
				...
				if (jet.length < 6) {
					...
					soin = Formulas.getRandomJet(jet[5]);
				}
				int dmg2 = soin;
				for (Fighter target : cibles) {
					soin = getMaxMinSort(target, soin);
					int soinFinal = Formulas.calculFinalSoin(caster, soin);
					if ((soinFinal + target.getPDV()) > target.getPDVMAX())
						soinFinal = target.getPDVMAX() - target.getPDV();
					if (soinFinal < 1)
						soinFinal = 0;
					target.removePDV(-soinFinal);
		...
		private void applyEffect_177(ArrayList<Fighter> cibles, Fight fight) {
			int val = Formulas.getRandomJet(jet);
			if (val == -1) {
				Log.addToLog("Erreur de valeur pour getRandomJet (applyEffect_"+effectID+")");
				return;
			...
			for (Fighter target : cibles) {
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
						Constants.STATS_REM_PROS, caster.getId() + "",
		...
		private void applyEffect_267(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_REM_VITA, caster.getId()+"", target.getId()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Constants.STATS_ADD_VITA, vol, turns, 1, false, spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_ADD_VITA, caster.getId()+"", caster.getId()+","+vol+","+turns);
		}
		...
		private void applyEffect_268(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_REM_AGIL, caster.getId()+"", target.getId()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Constants.STATS_ADD_AGIL, vol, turns, 1, false, spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_ADD_AGIL, caster.getId()+"", caster.getId()+","+vol+","+turns);
		}
		...
		private void applyEffect_269(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_REM_INTE, caster.getId()+"", target.getId()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Constants.STATS_ADD_INTE, vol, turns, 1, false, spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_ADD_INTE, caster.getId()+"", caster.getId()+","+vol+","+turns);
		}
		...
		private void applyEffect_270(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_REM_SAGE, caster.getId()+"", target.getId()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Constants.STATS_ADD_SAGE, vol, turns, 1, false, spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_ADD_SAGE, caster.getId()+"", caster.getId()+","+vol+","+turns);
		}
		...
		private void applyEffect_271(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_REM_FORC, caster.getId()+"", target.getId()+","+val+","+turns);
				vol += val;
			}
			if(vol == 0)return;
			//on ajoute le buff
			caster.addBuff(Constants.STATS_ADD_FORC, vol, turns, 1, false, spell, args, caster);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constants.STATS_ADD_FORC, caster.getId()+"", caster.getId()+","+vol+","+turns);
		}
		...
		private void applyEffect_149(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				if(id == -1)id = target.getGfx();
				target.addBuff(effectID, id, turns, 1, false, spell, args, caster);
				int defaut = target.getGfx();
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, effectID, caster.getId()+"", target.getId()+","+defaut+","+id+","+turns);
			}	
		}
		...
		private void applyEffect_150(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 150, caster.getId()+"", target.getId()+",4");
		...
		private void applyEffect_144(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					dmg = -(dmg);
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, caster.getId()+"", target.getId()+","+dmg);
				}
			}else
			{
				for(Fighter target : cibles)
		...
		private void applyEffect_400(Fight fight)
		{
			...
			for(Piege p :fight.getTraps())if(p.get_cell().getId() == cell.getId())return;
			...
			int unk = g.get_color();
			int team = caster.getTeam().getId()+1;
			String str = "GDZ+"+cell.getId()+";"+size+";"+unk;
		...
		private void applyEffect_180(Fight fight)//invocation
		{
			...
			Fighter F = new Fighter(fight,Clone,caster.getTeam());
			F.setInvocator(caster);
			fight.getMap().getCases().get(cell).addFighter(F);
			F.setFightCell(fight.getMap().getCases().get(cell));
            fight.getOrdreJeu().addFighter(F,caster);
			fight.addFighterInTeam(F, caster.getTeam());
			String gm = F.getGmPacket('+').substring(3);
            String gtl = fight.getOrdreJeu().generateGTLPaquet();
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 180, caster.getId() + "", gm);
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 999, caster.getId()+"", gtl);
			ArrayList<Piege> P = (new ArrayList<Piege>());
			P.addAll(fight.getTraps());
			for(Piege p : P)
			{
				int dist = Pathfinding.getDistanceBetween(fight.getMap(),p.get_cell().getId(),F.get_fightCell(false).getId());
				//on active le piege
		...
		private void applyEffect_181(Fight fight)//invocation
		{
			...
			if(mobID == -1 || level == -1 || MG == null)return;
            int id = fight.getMap().getNextFreeId(IdType.CREATURE);
            MG.setStatByInvocator(caster);
            Mob mob = new Mob(id,caster.getMap().getCases().get(cell),MG);
            Fighter F = new Fighter(fight,mob,caster.getTeam());
			F.setInvocator(caster);
			fight.getMap().getCases().get(cell).addFighter(F);
			F.setFightCell(fight.getMap().getCases().get(cell));
		...
		private void applyEffect_111(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			for(Fighter target : cibles)
			{
				...
				if(target.canPlay() && target == caster) target.setCurPA(fight, target.getDefaultPA()+val);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, effectID, caster.getId()+"", target.getId()+","+val+","+turns);
			}			
		}
		...
		private void applyEffect_128(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			for(Fighter target : cibles)
			{
				...
				if(target.canPlay() && target == caster) target.setCurPM(fight, target.getDefautPM()+val);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, effectID, caster.getId()+"", target.getId()+","+val+","+turns);
			}			
		}
		...
		private void applyEffect_77(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			for(Fighter target : cibles)
			{
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Constants.STATS_REM_PM, caster.getId()+"", target.getId()+",-"+val+","+turns);
				num += val;
			}
			if(num != 0)
			{
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Constants.STATS_ADD_PM, caster.getId()+"", caster.getId()+","+num+","+turns);
				caster.addBuff(Constants.STATS_ADD_PM, num, 0, 0, true, spell,args,caster);
				//Gain de PM pendant le tour de jeu
				if(caster.canPlay()) caster.setCurPM(fight, caster.getDefautPM()+num);
			}
		}
		...
		private void applyEffect_84(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			for(Fighter target : cibles)
			{
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Constants.STATS_REM_PA, caster.getId()+"", target.getId()+",-"+val+","+turns);
				num += val;
			}
			if(num != 0)
			{
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Constants.STATS_ADD_PA, caster.getId()+"", caster.getId()+","+num+","+turns);
				caster.addBuff(Constants.STATS_ADD_PA, num, 0, 0, true, spell,args,caster);
				//Gain de PA pendant le tour de jeu
				if(caster.canPlay()) caster.setCurPA(fight, caster.getDefaultPA()+num);
			}
		}
		...
		private void applyEffect_168(Fight fight, ArrayList<Fighter> cibles)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 168,target.getId()+"",target.getId()+",-"+value);
				}
			}else
			{
				for(Fighter target : cibles)
				{
					...
					else if(spell == 115)//Odorat
                    {
                        ...
                        if(turns <= 1 || duration <= 1)
                                SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 168,target.getId()+"",target.getId()+",-"+lostPa);
                    }
					else
					{
						target.addBuff(effectID, value, 1, 1, false, spell, args, caster);
		...
		private void applyEffect_169(Fight fight, ArrayList<Fighter> cibles)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					target.addBuff(effectID, value, 1, 1, false, spell, args, caster);
					if(turns <= 1 || duration <= 1)
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 169,target.getId()+"",target.getId()+",-"+value);
				}
			}else
			{
				if(cibles.isEmpty() && spell == 120 && caster.get_oldCible() != null)
				{
					...
					if(turns <= 1 || duration <= 1)
						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 169,caster.get_oldCible().getId()+"",caster.get_oldCible().getId()+",-"+value);
				}
				for(Fighter target : cibles)
				{
					...
					else if(spell == 115)//Odorat
                    {
                        ...
                        if(turns <= 1 || duration <= 1)
                                SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 169,target.getId()+"",target.getId()+",-"+lostPm);
                    }
					else
					{
						target.addBuff(effectID, value, 1, 1, false, spell, args, caster);
		...
		private void applyEffect_4(Fight fight,ArrayList<Fighter> cibles)
		{
			...
			if(cell.isWalkable(true) && !fight.isOccuped(cell.getId()))//Si la case est prise, on va �viter que les joueurs se montent dessus *-*
			{
				...
				P.addAll(fight.getTraps());
				for(Piege p : P) {
					int dist = Pathfinding.getDistanceBetween(fight.getMap(), p.get_cell().getId(), caster.get_fightCell(false).getId());
					//on active le piege
		...
		private void applyEffect_6(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					int newCellID =	Pathfinding.newCaseAfterPush(fight.getMap(),eCell,target.get_fightCell(false),-value);
					if(newCellID == 0)
						return;
					...
					if(newCellID <0)//S'il a �t� bloqu�
					{
						...
						newCellID =	Pathfinding.newCaseAfterPush(fight.getMap(),caster.get_fightCell(false),target.get_fightCell(false),a);
						if(newCellID == 0)
							return;
						if(fight.getMap().getCases().get(newCellID) == null)
							return;
					}
					
					target.get_fightCell(false).getFighters().clear();
					target.setFightCell(fight.getMap().getCases().get(newCellID));
					target.get_fightCell(false).addFighter(target);
		...
		private void applyEffect_130(ArrayList<Fighter> cibles, Fight fight) {// vole de kamas
			...
			for (Fighter target : cibles) {
			Player perso = target.getPersonnage();
			...
			Player perso2 = caster.getPersonnage();
			if (perso2 != null) {
			perso2.setKamas(perso2.getKamas() + val);
			}
			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 130, caster.getId() + "", val + "");
			}
			try {
			Thread.sleep(200);
			} catch (InterruptedException e1) {}
		...
		private void applyEffect_176(ArrayList<Fighter> cibles, Fight fight) { 
			int val = Formulas.getRandomJet(jet);
			if (val == -1) {
				return;
			}
			for (Fighter target : cibles) {
				target.addBuff(effectID, val, turns, 1, true, spell, args, caster);
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,
						Constants.STATS_ADD_PROS, caster.getId() + "",
						target.getId() + "," + val + "," + turns);
			}
			try {
				Thread.sleep(200);
		...
		private void applyEffect_108(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			if(turns <= 0)
			{
				...
				for(Fighter target : cibles)
				{
					...
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, caster.getId()+"", target.getId()+",+"+finalSoin);
				}
			}else
			{
				for(Fighter target : cibles)
		...
		private void applyEffect_141(final Fight fight,ArrayList<Fighter> cibles)
		{
			for(Fighter target : cibles)
			{
                ...
                GlobalThread.registerAction(new ExecuteOnceAction(1500) {
                    @Override
                    public void applyAction() {
                        fFighter.removePDV(fFighter.getPDV());
                    }
                });
			}
		}
		...
		private void applyEffect_320(Fight fight, ArrayList<Fighter> cibles)
		{
			...
			for(Fighter target : cibles)
			{
				...
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Constants.STATS_REM_PO, caster.getId()+"", target.getId()+","+value+","+turns);
				num += value;
			}
			if(num != 0)
			{
				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7,Constants.STATS_ADD_PO, caster.getId()+"", caster.getId()+","+num+","+turns);
		...
		private void applyEffect_780(Fight fight)
		{
			...
			if(!target.isInvocation()) 
				...
				fight.getOrdreJeu().addFighter(target,target.getInvocator());

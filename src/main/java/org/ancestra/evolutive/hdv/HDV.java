...
public class HDV {
	...
	{
		private int ligneID;
		private ArrayList<ArrayList<HdvEntry>> _entries = new ArrayList<ArrayList<HdvEntry>>(3);//La premi�re ArrayList est un tableau de 3 (0=1 1=10 2=100 de quantit�)
		...
		public ArrayList<HdvEntry> getAll()
		{
			...
			for (int qte = 0; qte < _entries.size(); qte++)//Boucler dans les quantit�
			{
		...
		public boolean isEmpty()
		{
			for (int i = 0; i < _entries.size(); i++) 
			{
				try
				{
					if(_entries.get(i).get(0) != null)//V�rifie s'il existe un objet dans chacune des 3 quantit�
						return false;

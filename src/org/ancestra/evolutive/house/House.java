...
public class House {
				...
				if(house.getValue().getGuildId() > 0) {
					...
					if(guild != null)
					{
						...
						if(guild.getMembers().size() < 10) {//Ce n'est plus une maison de guilde
							...
							if(player.getGuild() != null && player.getGuild().getId() == house.getValue().getGuildId() && house.getValue().canDo(Constants.H_GBLASON))//meme guilde
								packet.append(";").append(name).append(";").append(emblem);
							else
							if(house.getValue().canDo(Constants.H_OBLASON))//Pas de guilde/guilde-différente
								packet.append(";").append(name).append(";").append(emblem);
						}
					}
				}
	...
	public void open(Player player) {
		...
		if(house == null) 
			return;
		if(house.getOwner() == player.getAccount().getUUID() || (player.getGuild() != null && player.getGuild().getId() == house.getGuildId() && canDo(Constants.H_GNOCODE)))//C'est sa maison ou même guilde + droits entrer sans pass
	...
	public static void buy(Player player) {
		...
		for(Player z: player.getCurMap().getPlayers())
			House.load(z, z.getCurMap().getId());
	}
	...
	public static void leave(Player player, String packet) {
		...
		if(target == null || !target.isOnline() || target.getFight() != null || target.getCurMap().getId() != player.getCurMap().getId()) 
			return;

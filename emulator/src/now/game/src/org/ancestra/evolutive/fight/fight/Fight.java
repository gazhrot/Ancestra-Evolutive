...
public abstract class Fight {
    ...
    public void set_curFighterPM(int _curFighterPM) {
        this._curFighterPM = _curFighterPM;
    }
	...
	private synchronized void startTurn(){
        ...
        set_curFighterPA(this.currentFighter.getDefaultPA());
        set_curFighterPM(this.currentFighter.getDefautPM());
        _curFighterUsedPA = 0;
    ...
    public void joinFight(Player player,Team team){
        if(team == null || !team.canJoin(player)) {
            send("GA;903;"+player.getId()+";f");
            return;
        }
    ...
    public void onSpectatorBehaviourChange(PlayerTeam playerTeam){
        ...
        if(!specOk){
            for(Player player : spectator.values()){
                ...
                player.getGameActionManager().resetActions();
	...
	public int fighterDeplace(Fighter f, AtomicReference<String> path,int actionId) {
        if(currentFighter == null || f != currentFighter || state != FightState.ACTIVE)return -1;
        if(path.get().equals("")) {
		...
		if(tacle != null && !f.isState(6))//Tentative de Tacle : Si stabilisation alors pas de tacle possible
		{
			...
			if(!tacle.isEmpty())//Si tous les tacleur ne sont pas stabilis?s
			{
				...
				if(rand > chance) {
					send("GA;104;" + currentFighter.getIa());//Joueur tacl?
					int pertePA = get_curFighterPA()*chance/100;
					currentFighter.removePM(currentFighter.getPM());
					currentFighter.removePA(pertePA);
		...
		//*
		int nStep = Pathfinding.isValidPath(map, f.get_fightCell(false).getId(), path, Fight.this);
		String newPath = path.get();
        if( nStep > currentFighter.getPM() || nStep == -1000){
		...
		if(currentFighter.getFighterType() == Fighter.FighterType.PLAYER)
			send("GAS" + currentFighter.getId());
        final String message = "GA" + actionId + ";1;" + currentFighter.getId() + ";a"+CryptManager.cellID_To_Code(f.get_fightCell(false).getId())+newPath;
        if(!currentFighter.isHide())
            this.send(message);
        else//Si le joueur est planqu? x)
            currentFighter.send(message);
       
        //Si port?
        Fighter po = currentFighter.get_holdedBy();
        if(po != null && currentFighter.isState(Constants.ETAT_PORTE) && po.isState(Constants.ETAT_PORTEUR)){
        	logger.debug("Porteur: " + po.getName());
        	logger.debug("NextCellID " + nextCellID);
        	logger.debug("Cell du Porteur " + po.get_fightCell(false).getId());
        ...
        && po.isState(Constants.ETAT_PORTE)){
        	...
        	logger.debug(po.getName() + " se deplace vers la case " + nextCellID);
      	}
        return nStep;
    }

	public void onGK(Player perso,int step){
		if(currentFighter != perso.getFighter() || state != FightState.ACTIVE)return;
		logger.debug("Fin du deplacement de Fighter ID= " + perso.getId());
        currentFighter.removePM(step);
	...
	public void playerPass(Player _perso) {
		...
		if(f == null || f != currentFighter)return;
		endTurn();
	}

    public void tryCastSpell(Fighter fighter,SpellStats spell, int caseId){
        tryCastSpell(fighter,spell,caseId,-1);
    }

	public void tryCastSpell(Fighter fighter,SpellStats Spell, int caseID,int actionId){
		Case Cell = map.getCases().get(caseID);

		if(!CanCastSpell(fighter,Spell,Cell, -1)) return;
        if (fighter.getPersonnage() != null)
            SocketManager.GAME_SEND_STATS_PACKET(fighter.getPersonnage());

        logger.trace(fighter.getName() + " tentative de lancer le sort " + Spell.getSpellID() + " sur la case " + caseID);
        final boolean isEc = Spell.getTauxEC() != 0 && Formulas.getRandomValue(1, Spell.getTauxEC()) == Spell.getTauxEC();
        if (isEc) {
            logger.trace(fighter.getName() + " Echec critique sur le sort " + Spell.getSpellID());
            fighter.getFight().send("GA;302;" + fighter.getId() + ";" + Spell.getSpellID());
        } else {
            ...
            fighter.getFight().send("GA;300;" + fighter.getId() + ";" + sort);
            if (isCC) {
                logger.trace(fighter.getName() + " Coup critique sur le sort " + Spell.getSpellID());
                fighter.getFight().send("GA;301;" + fighter.getId());
            }
        ...
        fighter.removePA(Spell.getPACost());
        if(actionId != -1) {
            fighter.getFight().send("GAF" + actionId + "|" + fighter.getId());
        }
        if (isEc && Spell.isEcEndTurn())
            fighter.getFight().endTurn();
	...
	public boolean CanCastSpell(Fighter fighter, SpellStats spell, Case cell, int launchCase){
		...
		if(spell.getSpellID() == 67)
			if(!Pathfinding.checkLoS(map, Pathfinding.GetCaseIDFromDirection(ValidlaunchCase, dir, map, true), cell.getId(), null, true, getAllFighters())) {
    ...
    public void onFightEnded(Team winner, Team looser){
        ...
        this.send(this.getGE(winner, looser));
	...
	public void onFighterDie(Fighter target) {
        ...
        this.send("GA;103;"+ target.getId() + ";" + target.getId());
	...
	public void leftFight(Player target) {
		...
		if(T != null) {
            ...
            target.getGameActionManager().resetActions();
            target.send(this.getGE(otherTeam, playerTeam));

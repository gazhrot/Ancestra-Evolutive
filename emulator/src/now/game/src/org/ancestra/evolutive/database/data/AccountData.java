...
public class AccountData extends AbstractDAO<Account>{
	...
	public boolean canMigrate(int account) {
        try {
        	boolean canMigrate = false;
            String query = "SELECT * FROM accounts WHERE guid = '" + account + "';";
            Result result = super.getData(query);
            
            if(result.resultSet.next())
            	canMigrate = result.resultSet.getInt("migration") == 0 ? false : true;
    ...
    protected Account loadFromResultSet(ResultSet resultSet) throws SQLException {
        if(resultSet.next()) {
            ...
            World.database.getPlayerData().load(account);
            return account;
        }
        return null;
    }

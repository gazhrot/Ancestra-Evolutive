...
public class Case {
	...
	public void startAction(Player perso, GameAction GA)
	{
		...
		switch(actionID)
		{
			...
			case 102://Puiser
				if(!this.interactiveObject.isInteractive())return;//Si l'objet est utilisé
			...
			case 157: //Zaapis
				...
				if (perso.getCurMap().getSubArea().getArea().getId() == 7 && (perso.getAlign() == 1 || perso.getAlign() == 0 || perso.getAlign() == 3))//Ange, Neutre ou Sérianne
				{
					Zaapis = Constants.ZAAPI.get(Constants.ALIGNEMENT_BONTARIEN).split(",");
					if (perso.getAlign() == 1) price = 10;
				}
				else if (perso.getCurMap().getSubArea().getArea().getId() == 11 && (perso.getAlign() == 2 || perso.getAlign() == 0 || perso.getAlign() == 3))//Démons, Neutre ou Sérianne
				{
			...
			case 183://Retourner sur Incarnam
				...
				short mapID  = Constants.getStartMap(perso.getClasse());
				int cellID = Constants.getStartCell(perso.getClasse());

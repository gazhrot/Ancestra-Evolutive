...
public class Forget implements PacketParser {
	...
	public void parse(GameClient client, String packet) {
		...
		if(Server.config.isDebug()) 
			Log.addToLog("Info: "+client.getPlayer().getName()+": Tente Oublie sort id="+id);	
		if(client.getPlayer().forgetSpell(id)) {
			if(Server.config.isDebug()) 
				Log.addToLog("Info: "+client.getPlayer().getName()+": OK pour Oublie sort id="+id);
			...
			client.getPlayer().setForgetingSpell(false);
		}
	}
}
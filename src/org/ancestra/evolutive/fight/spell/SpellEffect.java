...
public class SpellEffect {
	...
	private Case cell = null;
				...
				for(SpellEffect buff : target.getBuffsByEffectID(id))
				{
					switch(id)
					{
						case 9://Derobade
							...
							int dist = Pathfinding.getDistanceBetween(fight.get_map(), target.get_cell().getId(), caster.get_cell().getId());
							if (dist > 1)
								continue;
							int cellNumbers = 0;
							try {
		...
		public void applyToFight(Fight fight,Fighter acaster, ArrayList<Fighter> cibles, boolean isCaC)
		{
			...
			switch(effectID)
			{
				...
				case 671://Dommages : X% de la vie de la victime (neutre)
					applyEffect_671(cibles,fight); // Combustion SpontanÃ©e
		...
		private void applyEffect_185(Fight fight)
		{
			...
			fight.get_map().getCases().get(cellID).addFighter(F);
			F.set_fightCell(fight.get_map().getCases().get(cellID));
		...
		private void applyEffect_783(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			Case tcase = fight.get_map().getCases().get(tcellID);
		...
		private void applyEffect_144(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					dmg = applyOnHitBuffs(dmg,target,caster,fight);//S'il y a des buffs spÃ©ciaux
		...
		private void applyEffect_180(Fight fight)//invocation
		{
			...
			F.setTeam(caster.getTeam());
			F.setInvocator(caster);
			fight.get_map().getCases().get(cell).addFighter(F);
			F.set_fightCell(fight.get_map().getCases().get(cell));
			...
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					if(newCellID <0)//S'il a été bloqué
					{
						...
						if(newCellID == 0)
							return;
						if(fight.get_map().getCases().get(newCellID) == null)
							return;
					}
					
					target.get_fightCell(false).getFighters().clear();
					target.set_fightCell(fight.get_map().getCases().get(newCellID));
					target.get_fightCell(false).addFighter(target);

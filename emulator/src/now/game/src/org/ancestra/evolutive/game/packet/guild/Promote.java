...
public class Promote implements PacketParser {
	...
	public void parse(GameClient client, String packet) {
		...
		else	//Sinon, c'est un membre normal
		{
			...
			{
				if(!changer.canDo(GuildRight.SET_RANK.getId()) || rank == 1)	//S'il ne peut changer les rang ou qu'il veut mettre meneur
					rank = -1; 	//"Reset" le rang
				
				if(!changer.canDo(GuildRight.SET_RIGHT.getId()) || right == 1)	//S'il ne peut changer les droits ou qu'il veut mettre les droits de meneur
					right = -1;	//"Reset" les droits
				
				if(!changer.canDo(GuildRight.HIS_XP.getId()) && !changer.canDo(GuildRight.ALL_XP.getId()) && changer.getUUID() == toChange.getUUID())	//S'il ne peut changer l'XP de personne et qu'il est la cible
					xpGive = -1; //"Reset" l'XP
			}
			
			if(!changer.canDo(GuildRight.ALL_XP.getId()) && !changer.equals(toChange))	//S'il n'a pas le droit de changer l'XP des autres et qu'il n'est pas la cible
				xpGive = -1; //"Reset" L'XP
		}

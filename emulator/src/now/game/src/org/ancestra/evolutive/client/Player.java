...
public class Player extends Fightable implements Serializable {
	...
	public static Player create(String name, int sex, int classeId, int color1, int color2, int color3, Account compte) {
        ...
        Player player = new Player(
				World.database.getPlayerData().nextId(),
				name,
				sex,
				classeId,
				color1,
        ...
        if(!World.database.getPlayerData().create(player))
			return null;
	...
	public void setSex(int sex) {
		this.sex = sex;
		World.database.getPlayerData().updateSex(this);
	}
	...
	public void setFight(Fight fight) {
        ...
        this.fight = fight;
        this.ready = false;
        this.getGameActionManager().resetActions();
    ...
    public void onStartTurn(Fighter fighter) {
        this.ready = false;
        this.getGameActionManager().setStatus(GameActionManager.Status.WAITING);
	...
	public int getPdv() {
        ...
        if(fight != null || regenRate == 0){
            return this.pdv;
        }
        this.setPdv((int)time/regenRate + this.pdv);
		return this.pdv;
	}
	...
	public void addZaap(int mapId) {
		if(!this.zaaps.contains(Integer.valueOf(mapId)))	{
			...
			World.database.getPlayerData().update(this);
		}
	}
	...
	public void remove() {
		World.database.getPlayerData().delete(this);
	}
	...
	public String getAsPacket() {
        ...
        int pdv,pdvMax;
        if(this.getFight() != null && this.getFighter() != null) {
            pdv = this.getFighter().getPDV();
            pdvMax = this.getFighter().getPDVMAX();
        } else {
            pdv = this.getPdv();
            pdvMax = this.getMaxPdv();
        }
	...
	public void levelUp(boolean send,boolean addXp)
	{
		...
		if(addXp)
			this.experience = World.data.getExpLevel(this.getLevel()).player;
	...
	public void finishActionOnCell(int cell,int action){
		this.getMap().getCases().get(cell).finishAction(this, action);
	}
	...
	public void openMountPark() {
		...
		this.getGameActionManager().setStatus(GameActionManager.Status.EXCHANGING);
		...
		if(this.getGuildMember() != null) {
			if(World.data.getPlayer(this.getCurMountPark().getOwner()).getGuildMember() != null)
				if(World.data.getPlayer(this.getCurMountPark().getOwner()).getGuildMember().getGuild() == this.getGuildMember().getGuild() && getGuildMember().canDo(GuildRight.USE_PARK.getId()))
					SocketManager.GAME_SEND_ECK_PACKET(this, 16, str);
		} else {
	...
	public boolean doJobAction(int actionID, InteractiveObject object, int GAId,Case cell){
		JobStat SM = getMetierBySkill(actionID);
		if(SM == null)return false;
		return SM.startAction(actionID,this, object,GAId,cell);
	}
	public void finishJobAction(int actionID, InteractiveObject object,Case cell)
	{
		JobStat SM = getMetierBySkill(actionID);
		if(SM == null)return;
		SM.endAction(actionID,this, object,cell);
	}
	...
	public void setGhosts()
	{
		...
		this.getGameActionManager().setStatus(GameActionManager.Status.GHOST);
		this.setSpeed(-40);
    ...
    public void setSitted(boolean sitted){
        ...
        this.sitted = sitted;
        stopLifeTimer();
        startLifeTimer(sitted ? 1000 : 2000);
    }

    public void stopLifeTimer(){
        final long time = (System.currentTimeMillis()-regenTime);
        final int oldPdv = this.pdv;
        if(fight != null || regenRate == 0){
            return;
        }
        int lifeEarned = (int)time/regenRate;
        this.setPdv(this.pdv + lifeEarned);
        this.regenRate = 0;
        lifeEarned = this.pdv - oldPdv;
        if(lifeEarned != 0)
            this.send("ILF" + lifeEarned);
    }

    public void startLifeTimer(int regenRate) {
        this.regenTime = System.currentTimeMillis();
        this.regenRate = regenRate;
        this.send("ILS" + this.regenRate);
    }
    ...
    /**
     * Affiche le message dans une bulle
     * @param message message a afficher
     */
    public void say(String message) {
        this.getMap().send("cMK|" + this.getId()+ "|" + this.getName() + "|" + message);
    }
    ...
    public boolean onMapChange(Case oldCell ,Case newCell){
        if(!this.followers.isEmpty())//On met a jour la carte des personnages qui nous suivent
            for(Player player : this.followers.values())
                if(player.isOnline())
                    player.send("IC" + newCell.getMap().getX() + "|" + newCell.getMap().getY());
                else
    ...
    public PlayerMigration serialize() {
    	return new PlayerMigration(this.getId(), this.getMount(), this.getObjects().values());
    }
}
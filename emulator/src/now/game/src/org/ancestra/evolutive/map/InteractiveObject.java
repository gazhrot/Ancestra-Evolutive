...
public class InteractiveObject {
	...
	public InteractiveObject(int id, final Maps map, final Case cell) {
        ...
        this.walkable = this.getTemplate() != null &&
                (this.getTemplate().isWalkable() || this.state == JobConstant.IOBJECT_STATE_EMPTY);
		...
		switch(this.getId()) {			
			...
			case 684://Portillon donjon squelette.
				if(!player.hasItemTemplate(1570, 1)) {
					player.sendText("Vous ne poss�dez pas la clef nec�ssaire.");
				} else {
	...
	private void send(Player player, String packet) {
		player.send(packet);
		player.getGameActionManager().setStatus(GameActionManager.Status.DIALOG);
	}

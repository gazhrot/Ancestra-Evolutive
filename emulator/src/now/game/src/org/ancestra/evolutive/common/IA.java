...
//		Thread.currentThread().interrupt();
	...
	public boolean moveFarIfPossible(Fight fight, Fighter F)
	{
		...
		if(F.getDefautPM() <= 0)return false;
		...
		GA.setArgs(pathstr);
		int step = fight.fighterDeplace(F,new AtomicReference<>(GA.getArgs()),GA.getId());
        fighter.removePM(step+1);
        return step != -1;

	}

	public boolean testCotes(int cell1, int cell2)
	{
	...
	public SpellStats getInvocSpell(Fight fight,Fighter fighter,int nearestCell)
	{
		for(Entry<Integer, SpellStats> SS : fighter.getSpells().entrySet())
		{
	...
	public boolean HealIfPossible(Fight fight, Fighter f, boolean autoSoin)//boolean pour choisir entre auto-soin ou soin alli�
	{
		...
		else//s�lection joueur ayant le moins de pv
		{
			...
			{					
				...
				if(F.getTeam() == f.getTeam())
				{
					...
					{
						int infl = 0;
						for(Entry<Integer, SpellStats> ss : f.getSpells().entrySet())
						{
		...
		fight.tryCastSpell(f, SS, target.get_fightCell(true).getId());
		return true;
	}
	
	public boolean HealIfPossiblePerco(Fight fight, Fighter f, boolean autoSoin)//boolean pour choisir entre auto-soin ou soin alli�
	{
		if(autoSoin && (f.getPDV()*100)/f.getPDVMAX() > 95 )return false;
		Fighter target = null;
		SpellStats SS = null;
	...
	{		
		...
		if(SS == null)return false;
		fight.tryCastSpell(fighter, SS, target.get_fightCell(true).getId());
		return true;
	}
	
	public boolean buffIfPossiblePerco(Fight fight, Fighter fighter,Fighter target)
	{		
		if(target == null)return false;
	...
	public SpellStats getBuffSpell(Fight fight, Fighter F, Fighter T)
	{
		int infl = 0;	
		SpellStats ss = null;
		for(Entry<Integer, SpellStats> SS : F.getSpells().entrySet())
	...
	private boolean moveNearIfPossible(Fight fight, Fighter F, Fighter T)
	{
		...
		if(Server.config.isDebug()) Log.addToLog("Tentative d'approche par "+F.getName()+" de "+T.getName());
	...
	{	
		...
		SpellStats SS2 = null;
		for(Entry<Integer, SpellStats> S : fighter.getSpells().entrySet())
		{
		...
		{
			fight.tryCastSpell(fighter, SS2, cell);
	...
	private boolean moveToAttackIfPossible(Fight fight,Fighter fighter)
	{
		...
		GameAction GA = new GameAction(0,1, "");
		GA.setArgs(pathstr);
        int step = fight.fighterDeplace(fighter, new AtomicReference<>(GA.getArgs()),GA.getId());
		fighter.removePM(step+1);
        return step != -1;
	}
	...
	{
		ArrayList <SpellStats> sorts = new ArrayList <>();
		for(Entry<Integer, SpellStats> S : fighter.getSpells().entrySet())
		{
        ...
        return TriInfluenceSorts(fighter,sorts);
	}
	...
	private SpellStats getBestSpellForTarget(Fight fight, Fighter F,Fighter T)
	{
		...
		for(Entry<Integer, SpellStats> SS : F.getSpells().entrySet())
		{
			...
			for(Entry<Integer, SpellStats> SS2 : F.getSpells().entrySet())
			{
				...
				for(Entry<Integer, SpellStats> SS3 : F.getSpells().entrySet())
				{
